...
class APIClient:
    def __init__(self):
        ...
        except KeyError(f"Unsupported environment value: {environment_str}"):
    ...
    def ping(self):
        ...
        with allure.step("Assert status code"):
            assert response.status_code == 201, f"Expected status_code 201 but got {response.status_code}"
        return response.status_code

    def auth(self):
        with allure.step("Auth api client"):
    ...
    def create_booking(self, booking_data):
        with allure.step("Сreate booking"):
    ...
    def get_booking_ids(self, params= None):
        with allure.step("Getting information by id"):
            url = f"{self.base_url}{Endpoints.BOOKING_ENDPOINT}"
