...
def test_ping(api_client):
    ...
    assert status_code == 201, f"Expected status_code 201 but got {status_code}"
...
def test_ping_wrong_method(api_client, mocker):
    ...
    with pytest.raises(AssertionError, match= f"Expected status_code 201 but got 405"):
        api_client.ping()


@allure.feature("Test Ping")
@allure.story("Test server error")
def test_internal_server_error(api_client, mocker):
    mock_response = mocker.Mock()
    mock_response.status_code = 500
    mocker.patch.object(api_client.session, "get", return_value = mock_response)
    with pytest.raises(AssertionError, match = f"Expected status_code 201 but got 500"):
        api_client.ping()
...
def test_ping_not_found(api_client, mocker):
    ...
    mocker.patch.object(api_client.session, "get", return_value = mock_response)
    with pytest.raises(AssertionError, match= f"Expected status_code 201 but got 404"):
        api_client.ping()


@allure.feature("Test Ping")
@allure.story("Test connection with different success code")
def test_ping_different_code(api_client, mocker):
    mock_response = mocker.Mock()
    mock_response.status_code = 200
    mocker.patch.object(api_client.session, "get", return_value = mock_response)
    with pytest.raises(AssertionError, match = f"Expected status_code 201 but got 200"):
        api_client.ping()
